...
事务是独立管线：**每条消息都经一次性 LLM 判定（脱稿不进会话，用户定稿
「别代码里判断」）→ 命中待办直接代办**。提醒是首个注册的事务种类：判定
为提醒请求时管线**直接登记进 reminders 存储**并注入「已代办」上下文——
角色只管用口吻转告「记下了」，想忘都忘不了；判定为提醒但时间没看懂时
注入「问清时间」上下文，不装没听见。

- 判定失败一律静默降级为普通回复，绝不拖垮主回复。
- 通用设计：`AttentionKind`（预筛 + 判定 prompt + 输出解析）可注册扩展，
  核心类在 `core/agent/attention.py`，与具体事务零耦合。
- 成本：每轮回复多一次短 JSON 判定调用（用户接受换取零漏判）。
