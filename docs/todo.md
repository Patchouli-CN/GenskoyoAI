...
## 8.64 回复对象标记：助手记忆带「（对 某人）」治张冠李戴（2026-08-03，用户实机发现）
...
- 机制：`helpers.extract_speaker_tags` 机械提取行首【昵称】标记
  （去重保序，行中出现的引用不算）；`_on_generate_response` 发布
  MESSAGE_SENT 时把触发消息的发言人名单放进 `data["reply_to"]`；
  `on_message_sent` 把工作记忆里的助手消息存为
  `（对 帕秋莉、赤色杀人魔）…`——**只写记忆，投递给用户的
  content 本体不动**；私聊/控制台/World 无标记自然为空，行为不变。
- 顺带厘清另两类「幻觉」不治本但定性：设定错误（觉妖怪=亡灵）靠
  角色卡常识锚点；装傻卖萌是特性。
- 测试：新文件 test_reply_to_tagging.py（提取 5 例 + 记忆写入 2 例）；
  相关 83 例全绿，ruff / pyright 全绿。

建议 commit message（用户已授权直接提交）：
`feat(agent): 回复对象标记——助手消息在工作记忆带「（对 某人）」前缀（提取触发消息的【昵称】行首标记），修多人快节奏对话的张冠李戴；投递文本不受影响`
