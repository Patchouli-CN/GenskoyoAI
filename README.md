...
记忆管理不是简单地“全部塞进上下文”，也不依赖模型自觉去记。系统会在对话推进中定期把近期对话自动蒸馏成值得长期保存的“珍贵记忆”（事实、偏好、关系变化、有情感重量的事件），并围绕话题建立联系；后续对话中，系统会检索相关记忆注入提示词，帮助角色更自然地想起过去内容。配合话题和遗忘机制，记忆更像真实交流中的印象，而不是僵硬的记录本。
